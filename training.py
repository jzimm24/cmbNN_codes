...
def training_step(images, ground_truths, generator, gen_optimizer, discriminator, disc_optimizer):
    ...
    # losses
    gen_loss = models.generator_loss(preds, ground_truths,img_preds_discrimination)
...
def training_loop(generator, gen_optimizer, discriminator, disc_optimizer, dataloader, num_epochs = NUM_EPOCHS):
    for epoch in range(num_epochs):
        for x, y in dataloader:
            current_gen_loss, current_disc_loss = training_step(x, y, generator, gen_optimizer, discriminator, disc_optimizer)
...
def main():
    ...
    generator.train()
    discriminator.train()
    generator, discriminator = training_loop(generator, generator_optimizer, discriminator, discriminator_optimizer, dataloader)
    return None
